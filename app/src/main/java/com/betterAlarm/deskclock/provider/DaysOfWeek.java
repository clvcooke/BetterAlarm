...
package com.betterAlarm.deskclock.provider;
...
public final class DaysOfWeek {
    ...
    private String toString(Context context, boolean showNever, boolean forAccessibility) {
        ...
        if (mBitSet == NO_DAYS_SET) {
            return showNever ? context.getText(com.betterAlarm.deskclock.R.string.never).toString() : "";
        }

        // every day
        if (mBitSet == ALL_DAYS_SET) {
            return context.getText(com.betterAlarm.deskclock.R.string.every_day).toString();
        }
        ...
        for (int bitIndex = 0; bitIndex < DAYS_IN_A_WEEK; bitIndex++) {
            if ((mBitSet & (1 << bitIndex)) != 0) {
                ...
                dayCount -= 1;
                if (dayCount > 0) ret.append(context.getText(com.betterAlarm.deskclock.R.string.day_concat));
            }
        }
        return ret.toString();
    }
