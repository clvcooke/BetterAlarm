...
import com.betterAlarm.deskclock.TimerRingService;
...
public class TimerReceiver extends BroadcastReceiver {
    ...
    private void showInUseNotification(final Context context) {
        ...
        if (timersInUse.size() == 1) {
            ...
            title = timerIsTicking ? label : context.getString(com.betterAlarm.deskclock.R.string.timer_stopped);
            ...
            if (timer == null) {
                // No running timers.
                title = String.format(
                        context.getString(com.betterAlarm.deskclock.R.string.timers_stopped), numTimersInUse);
                contentText = context.getString(com.betterAlarm.deskclock.R.string.all_timers_stopped_notif);
            } else {
                // We have at least one timer running and other timers stopped.
                title = String.format(
                        context.getString(com.betterAlarm.deskclock.R.string.timers_in_use), numTimersInUse);
                long completionTime = timer.getTimesupTime();
                long timeLeft = completionTime - now;
                contentText = String.format(context.getString(com.betterAlarm.deskclock.R.string.next_timer_notif),
    ...
    private static void showCollapsedNotification(final Context context, String title, String text,
        ...
        Notification.Builder builder = new Notification.Builder(context)
                ...
                .setShowWhen(false)
                .setSmallIcon(com.betterAlarm.deskclock.R.drawable.stat_notify_timer)
    ...
    private String buildTimeRemaining(Context context, long timeLeft) {
        ...
        String hourSeq = (hours == 0) ? "" :
            ( (hours == 1) ? context.getString(com.betterAlarm.deskclock.R.string.hour) :
                context.getString(com.betterAlarm.deskclock.R.string.hours, Long.toString(hours)) );
        String minSeq = (minutes == 0) ? "" :
            ( (minutes == 1) ? context.getString(com.betterAlarm.deskclock.R.string.minute) :
                context.getString(com.betterAlarm.deskclock.R.string.minutes, Long.toString(minutes)) );

        boolean dispHour = hours > 0;
        boolean dispMinute = minutes > 0;
        int index = (dispHour ? 1 : 0) | (dispMinute ? 2 : 0);
        String[] formats = context.getResources().getStringArray(com.betterAlarm.deskclock.R.array.timer_notifications);
    ...
    private void showTimesUpNotification(final Context context, TimerObj timerObj) {
        ...
        Notification notification = new Notification.Builder(context)
                ...
                .addAction(com.betterAlarm.deskclock.R.drawable.ic_menu_add,
                        context.getResources().getString(com.betterAlarm.deskclock.R.string.timer_plus_1_min),
                        addOneMinuteAction)
                .addAction(
                        timerObj.getDeleteAfterUse()
                                ? android.R.drawable.ic_menu_close_clear_cancel
                                : com.betterAlarm.deskclock.R.drawable.ic_notify_stop,
                        timerObj.getDeleteAfterUse()
                                ? context.getResources().getString(com.betterAlarm.deskclock.R.string.timer_done)
                                : context.getResources().getString(com.betterAlarm.deskclock.R.string.timer_stop),
                        stopIntent)
                .setContentTitle(timerObj.getLabelOrDefault(context))
                .setContentText(context.getResources().getString(com.betterAlarm.deskclock.R.string.timer_times_up))
                .setSmallIcon(com.betterAlarm.deskclock.R.drawable.stat_notify_timer)
                .setOngoing(true)
                .setAutoCancel(false)
