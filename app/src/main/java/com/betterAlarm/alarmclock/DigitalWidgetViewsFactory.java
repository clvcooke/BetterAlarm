...
import com.betterAlarm.deskclock.worldclock.CityObj;
import com.betterAlarm.deskclock.worldclock.WorldClockAdapter;
...
public class DigitalWidgetViewsFactory implements RemoteViewsFactory {
    ...
    private class RemoteWorldClockAdapter extends WorldClockAdapter {
        ...
        public RemoteWorldClockAdapter(Context context) {
            ...
            mClocksPerRow = context.getResources().getInteger(
                    com.betterAlarm.deskclock.R.integer.appwidget_world_clocks_per_row);
            mFontSize = context.getResources().getDimension(com.betterAlarm.deskclock.R.dimen.widget_medium_font_size);
            mFont24Size = context.getResources().getDimension(com.betterAlarm.deskclock.R.dimen.widget_24_medium_font_size);
        }
        ...
        public RemoteViews getViewAt(int position) {
            ...
            RemoteViews views = new RemoteViews(
                    mContext.getPackageName(), com.betterAlarm.deskclock.R.layout.world_clock_remote_list_item);

            // Always how the left clock
            updateView(views, (CityObj) mCitiesList[index], com.betterAlarm.deskclock.R.id.left_clock,
                    com.betterAlarm.deskclock.R.id.city_name_left, com.betterAlarm.deskclock.R.id.city_day_left);
            ...
            if (index + 1 < mCitiesList.length) {
                updateView(views, (CityObj) mCitiesList[index + 1], com.betterAlarm.deskclock.R.id.right_clock,
                        com.betterAlarm.deskclock.R.id.city_name_right, com.betterAlarm.deskclock.R.id.city_day_right);
            } else {
                hideView(views, com.betterAlarm.deskclock.R.id.right_clock, com.betterAlarm.deskclock.R.id.city_name_right,
                        com.betterAlarm.deskclock.R.id.city_day_right);
            }

            // Hide last spacer if last row
            int lastRow = ((mCitiesList.length + 1) / 2) - 1;
            if (position == lastRow) {
                views.setViewVisibility(com.betterAlarm.deskclock.R.id.city_spacer, View.GONE);
            } else {
                views.setViewVisibility(com.betterAlarm.deskclock.R.id.city_spacer, View.VISIBLE);
            }
        ...
        private void updateView(RemoteViews clock, CityObj cityObj, int clockId,
            ...
            WidgetUtils.setTimeFormat(clock,
                    (int)mResources.getDimension(com.betterAlarm.deskclock.R.dimen.widget_label_font_size), clockId);
            ...
            if (myDayOfWeek != cityDayOfWeek) {
                clock.setTextViewText(dayId, mContext.getString(
                        com.betterAlarm.deskclock.R.string.world_day_of_week_label, now.getDisplayName(
    ...
    public RemoteViews getViewAt(int position) {
        ...
        if (v != null) {
            ...
            v.setOnClickFillInIntent(com.betterAlarm.deskclock.R.id.widget_item, fillInIntent);
        }
        return v;
    }
