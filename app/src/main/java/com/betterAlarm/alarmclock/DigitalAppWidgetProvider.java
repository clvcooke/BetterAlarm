...
package com.betterAlarm.alarmclock;
...
import com.betterAlarm.deskclock.worldclock.Cities;
import com.betterAlarm.deskclock.worldclock.CitiesActivity;
...
public class DigitalAppWidgetProvider extends AppWidgetProvider {
     ...
     **/
    public static final String ACTION_ON_QUARTER_HOUR = "com.betterAlarm.deskclock.ON_QUARTER_HOUR";
                ...
                for (int appWidgetId : appWidgetIds) {
                    appWidgetManager.
                            notifyAppWidgetViewDataChanged(appWidgetId,
                                    com.betterAlarm.deskclock.R.id.digital_appwidget_listview);
                    RemoteViews widget = new RemoteViews(context.getPackageName(),
                            com.betterAlarm.deskclock.R.layout.digital_appwidget);
                    float ratio = WidgetUtils.getScaleRatio(context, null, appWidgetId);
                    WidgetUtils.setTimeFormat(widget, 0/*no am/pm*/, com.betterAlarm.deskclock.R.id.the_clock);
    ...
    private void updateClock(
        ...
        RemoteViews widget = new RemoteViews(context.getPackageName(), com.betterAlarm.deskclock.R.layout.digital_appwidget);
        ...
        if (newOptions != null &&
            ...
            widget.setOnClickPendingIntent(com.betterAlarm.deskclock.R.id.digital_appwidget,
        ...
        CharSequence dateFormat = DateFormat.getBestDateTimePattern(Locale.getDefault(),
                context.getString(com.betterAlarm.deskclock.R.string.abbrev_wday_month_day_no_year));
        widget.setCharSequence(com.betterAlarm.deskclock.R.id.date, "setFormat12Hour", dateFormat);
        widget.setCharSequence(com.betterAlarm.deskclock.R.id.date, "setFormat24Hour", dateFormat);
        ...
        widget.setRemoteAdapter(com.betterAlarm.deskclock.R.id.digital_appwidget_listview, intent);

        // Set up the click on any world clock to start the Cities Activity
        //TODO: Should this be in the options guard above?
        widget.setPendingIntentTemplate(com.betterAlarm.deskclock.R.id.digital_appwidget_listview,
                PendingIntent.
                        getActivity(context, 0, new Intent(context, CitiesActivity.class), 0));

        // Refresh the widget
        appWidgetManager.notifyAppWidgetViewDataChanged(
                appWidgetId, com.betterAlarm.deskclock.R.id.digital_appwidget_listview);
    ...
    protected void refreshAlarm(Context context, RemoteViews widget) {
        ...
        if (!TextUtils.isEmpty(nextAlarm)) {
            widget.setTextViewText(com.betterAlarm.deskclock.R.id.nextAlarm,
                    context.getString(com.betterAlarm.deskclock.R.string.control_set_alarm_with_existing, nextAlarm));
            widget.setViewVisibility(com.betterAlarm.deskclock.R.id.nextAlarm, View.VISIBLE);
            if (DigitalAppWidgetService.LOGGING) {
                Log.v(TAG, "DigitalWidget sets next alarm string to " + nextAlarm);
            }
        } else  {
            widget.setViewVisibility(com.betterAlarm.deskclock.R.id.nextAlarm, View.GONE);
            if (DigitalAppWidgetService.LOGGING) {
